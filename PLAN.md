...
> **最後更新**：2026-04-13
> **狀態總覽**：階段一 1.4-1.6 程式碼完成，等待 GPU 訓練產出 checkpoint
...
```
方向 A：PyTorch TextCNN          ← 程式碼完成，等 GPU 訓練
方向 B：BERT-Chinese（poc-bert/）  ← 程式碼建置中，可能直接切換
                                    兩者 GPU 訓練可同時進行
```
...
| 1.4 | **訓練流程**            | ✅ 程式碼完成       | train.py 已寫好 → **等你在 GPU 機器執行**      |
| 1.5 | **預測功能**            | ✅ 程式碼完成       | predict.py 已寫好 → 等 checkpoint 回來驗證   |
| 1.6 | **API 服務**          | ✅ 程式碼完成       | api.py 已寫好 → 等 checkpoint 回來啟動       |
| 1.7 | **測試與驗證**           | 🔨 Claude 可先做 | 撰寫 eval 腳本 → 等 checkpoint 回來執行       |
| 1.8 | **技術移轉文件**          | 🔨 Claude 可先做 | 架構圖、部署流程、API 規格文件                    |
...
| 2.3 | **BERT 訓練流程**            | ✅ 程式碼完成   | train.py → **等你在 GPU 機器執行**   |
| 2.4 | **BERT 預測 + API**        | ✅ 程式碼完成   | 相同 API 介面 → 等 checkpoint 回來驗證 |
| 2.5 | **BERT GPU 訓練指南**        | ✅ 完成      | docs/GPU訓練操作手冊_BERT.md        |
| 2.6 | 效能對比報告                   | ⬜ 等兩者都訓練完 | CNN vs BERT：準確度、速度、成本比較       |
